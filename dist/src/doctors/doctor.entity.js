"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Doctor = void 0;
const patient_entity_1 = require("../patients/patient.entity");
const clinic_entity_1 = require("../clinics/clinic.entity");
const swagger_1 = require("@nestjs/swagger");
const typeorm_1 = require("typeorm");
let Doctor = class Doctor {
};
exports.Doctor = Doctor;
__decorate([
    (0, swagger_1.ApiProperty)({ example: 1, description: 'Уникальный идентификатор' }),
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], Doctor.prototype, "id", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'Иванов Иван Иванович', description: 'ФИО' }),
    (0, typeorm_1.Column)({}),
    __metadata("design:type", String)
], Doctor.prototype, "fullname", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'Главврач', description: 'Должность' }),
    (0, typeorm_1.Column)(),
    __metadata("design:type", String)
], Doctor.prototype, "position", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Пациенты, закреплённые за врачом' }),
    (0, typeorm_1.ManyToMany)((type) => patient_entity_1.Patient, (patient) => patient.doctors),
    (0, typeorm_1.JoinTable)({
        name: 'doctor_patient',
        joinColumn: { name: 'doctor_id' },
        inverseJoinColumn: { name: 'patient_id' },
    }),
    __metadata("design:type", Array)
], Doctor.prototype, "patients", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Филиалы, в которых работает врач' }),
    (0, typeorm_1.ManyToMany)((type) => clinic_entity_1.Clinic, (clinic) => clinic.doctors),
    (0, typeorm_1.JoinTable)({
        name: 'doctor_clinic',
        joinColumn: { name: 'doctor_id' },
        inverseJoinColumn: { name: 'clinic_id' },
    }),
    __metadata("design:type", Array)
], Doctor.prototype, "clinics", void 0);
exports.Doctor = Doctor = __decorate([
    (0, typeorm_1.Entity)('doctors')
], Doctor);
//# sourceMappingURL=doctor.entity.js.map